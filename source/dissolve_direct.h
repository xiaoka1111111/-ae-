...
// 膨胀核 (0x369D0/0x36BB0 ): 3x3 max 滤波
//   规则: 未满像素 (值 < 1.0) 取邻域最大; 已满像素 (值 >= 1.0) 保留原值
//   (与 dilate shader 0x144801 #if0 设计语义一致; 每帧 1px = 切比雪夫传播)
...
//   P0=源左上, P1=源右上(垂直方向?), 原版语义: P0/P1 定义 x_src 方向, P2/P3 目标
...
//   4. 层着色: 实色/渐变(0x30B80)/源图 × 填充强度
//   5. 合成: overlay+lerp 0x30E50 数据序 (后画覆盖先画)
struct DirectFrame {
    const Preset* preset = nullptr;
    float progress01 = 0.f;          // 归一化进度 p01 = clamp((frame+1)/divisor, 0, 1) — 层 gating 用
    float totalFrames = 100.f;       // 归一化分母 (0x184F5) [C]
    float explicitFrames = -1.f;     // 已过帧数 (>=0 时直接用; <0 时 = p01×totalFrames−1)
                                     //   原版 fillMap 传播 = 帧号场 (frame01 帧膨胀), 与 gating 进度独立
    float splatRadius = 10.f;        // 圆点半径 param 28 (默认 10.0@0x1FEB88)
    float rampS = 1.f;               // 0x30B80 的 S = param 24 (噪波对比度)
    ...
    const float* noiseFill = nullptr;    // noiseMap (生长来源=噪波 的 0x30B80 fill)
    const float* shapeAlpha = nullptr;   // 形状 alpha (可选; 作为输出掩码把填充裁剪到图层不透明区。
                                         //   BFS 传播限在传播掩码内 (propMask 缺省 = 此场),
                                         //   点位于 alpha 外时波前被阻断 (原版 Borders))
    const float* propMask = nullptr;     // 传播掩码 (可选): BFS 只能在掩码>0 的像素内流动。
                                         //   默认 nullptr = 用 shapeAlpha 语义 (仅裁剪, 传播不受限);
                                         //   由 renderWorld 构建: 形状 alpha 经边界扩展/强度/桥接膨胀。
                                         //   语义对应原版 Borders: 填充不能穿越透明区域。
    float slotWeight = 1.f;              // 槽位权重 (0x1D4B0) [C]
    // 循环包络 [B]: 周期末尾平滑淡出系数 [0,1] (1=正常)。复刻循环 (fmod) 回绕时
    //   硬跳变 "满填充→1%" 观感像 bug; 85%→100% 淡出后回绕平滑。mode-3 源图层豁免
    //   (文字不能闪烁), 由 renderPresetDirect 内判断 L.mode==3。
    float loopEnv = 1.f;
    // 文字模式 (0=原版: mode-3 源图层 src-over 盖在填充上, 文字保持原色;
    //   1=填充覆盖文字: 跳过 mode-3 层, 彩色直接覆盖图层区域) [B]
    int sourceLayerMode = 0;
    // 混合模式 (0x3D830 跳表权威映射, A 级证据; 1-based popup 值, 见 dissolve_styles.h):
    //   1=正常(IN_FRONT) 3=正片叠底 4=颜色加深 6=相加 7=滤色 9=叠加 10=柔光 12=颜色
    //   14/15=模版Alpha/亮度 16/17=剪影Alpha/亮度; 0/非法值=正常
    int   blendMode = 1;
    // Fill_GPU (0x1425F1, 可选; 非空时对填充强度应用 speedOverlay+borderControl+gamma):
    //   9 步链 (0x16000, A 级, 调试路径与fill链.md): 1 warp累加 → 2/3 高斯模糊H/V
    //   (0x24F50: exp(-0.5·(x/R)²/0.06), 浮点半径 R/R+1 双核插值) → 4 gamma/exposure
    //   (0x33080: v>0.0001 ? clamp(pow(v,γ)·exp,0,1) : 0) → 5 speedOverlay (0x30DC0)
    //   → 6-8 清零/拷贝/膨胀(缓冲管线, 单场 max 累积等价) → 9 borderControl
    //   (0x30170: dst·(1−w·(1−src)))
    const float* speedMap = nullptr;     // 速度图 (0x30E50 的 src)
    float speedMapInfluenceF = 0.f;
    const float* edgeMap = nullptr;      // 边框图 (Sobel 0x146771)
